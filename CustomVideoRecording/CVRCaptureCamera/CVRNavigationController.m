...
@interface CVRNavigationController ()

@property (nonatomic, assign) BOOL isStatusBarHiddenBeforeShowCamera;

@end

@implementation CVRNavigationController
...
- (void)showCameraWithParentController:(UIViewController*)parentController block:(GetVideoBlock)block {
    CVRCameraController *con = [[CVRCameraController alloc] initWithBlock:^(NSURL *fileUrl, int state, NSString *errorInfo) {
        NSLog(@"get block = %@",fileUrl);
        block(fileUrl,state,errorInfo);
    }];
    con.maxSecond = self.maxSecond;
    con.minSecond = self.minSecond;
